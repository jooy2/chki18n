...
const KNOWN_LOCALES = new Set(ISO_639_1.split(' '));

/**
 * Whether a name looks like a locale. Region and script variants (`en-US`,
 * `pt_BR`, `zh-Hans`) are accepted by matching on the base language subtag.
 */
export function isLocaleCode(name: string): boolean {
	...
	return KNOWN_LOCALES.has(name.toLowerCase().split(/[-_]/)[0]);
}
