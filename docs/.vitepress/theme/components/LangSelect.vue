...
<template>
	<div ref="root" class="chki18n-lang-select">
		<p id="chki18n-lang-label" class="chki18n-lang-title">{{ t(locale, 'languageLabel') }}</p>

		<button
			ref="button"
			type="button"
			class="chki18n-lang-button"
			aria-haspopup="listbox"
			:aria-expanded="open"
			aria-labelledby="chki18n-lang-label chki18n-lang-current"
			@click="open ? closeMenu() : openMenu()"
			@keydown.down.prevent="openMenu()"
			@keydown.up.prevent="openMenu()"
		>
			<span id="chki18n-lang-current" class="chki18n-lang-current">
				<span
					v-for="item in CODE_LANGUAGES"
					:key="item.id"
					class="chki18n-lang chki18n-lang-face"
					:data-lang="item.id"
				>
			...
			<svg class="chki18n-lang-chevron" viewBox="0 0 24 24" aria-hidden="true">
				<path
					fill="none"
					stroke="currentColor"
					stroke-width="2"
		...
		<div v-if="open" class="chki18n-lang-menu" @keydown="onMenuKey">
			...
			<div class="chki18n-lang-list" role="listbox" :aria-label="t(locale, 'languageSelect')">
				<button
					v-for="item in CODE_LANGUAGES"
					:key="item.id"
					type="button"
					role="option"
					class="chki18n-lang-option"
					:aria-selected="item.id === codeLanguage"
					@click="choose(item.id)"
				>
					<LangMark :language="item.id" :size="15" />
					<span class="chki18n-lang-option-label">{{ item.label }}</span>
					<svg class="chki18n-lang-check" viewBox="0 0 24 24" aria-hidden="true">
						<path
							fill="none"
							stroke="currentColor"
							stroke-width="2.4"
			...
			<p class="chki18n-lang-hint">{{ t(locale, 'languageHint') }}</p>
		</div>
	</div>
</template>
