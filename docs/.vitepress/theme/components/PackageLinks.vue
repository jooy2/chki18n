...
/**
 * The navbar's Packages menu — one row per registry chki18n is published to.
 *
...
<template>
	<div class="chki18n-pkg" :class="{ 'chki18n-pkg-screen': screenMenu }">
		<a
			v-for="item in links"
			:key="item.id"
			class="chki18n-pkg-link"
			:href="item.url"
			target="_blank"
			rel="noopener"
			@click="closeScreen?.()"
		>
			...
			<span class="chki18n-pkg-registry">{{ item.registry }}</span>
			<span class="chki18n-pkg-lang">{{ labelOf(item.id) }}</span>
		</a>
	</div>
</template>
