 ...
 *
 * `<Lang js="interpolationPrefix" py="interpolation_prefix" code />`
 *
...
<template>
	<template v-for="variant in variants" :key="variant.id">
		<code v-if="code" class="chki18n-lang" :data-lang="variant.id">{{ variant.text }}</code>
		<span v-else class="chki18n-lang" :data-lang="variant.id">{{ variant.text }}</span>
	</template>
</template>
