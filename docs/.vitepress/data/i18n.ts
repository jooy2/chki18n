 ...
 * is the language switch above the sidebar.
 *
 * Option names are **not** in here on purpose. `interpolationPrefix` is what the
 * reader will type, so every page names the identifier rather than a translated
 * noun. What is left to translate is the chrome around them.
 */
...
const strings = {
	/*
	 * "Languages" rather than "Packages". The control offers JavaScript, Dart and
	 * Python — the languages chki18n ships for — and the registry each one is
	 * published to is a different question, answered by the navbar's Packages
	 * menu.
	 */
