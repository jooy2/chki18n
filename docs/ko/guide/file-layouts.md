...
번역 파일은 세 가지 방식 중 하나로 배치되며, chki18n은 따로 알려 주지 않아도 셋 모두를 인식합니다. 서로 다른 언어로 같은 키를 담고 있는 파일은 하나의 그룹으로 묶여 비교되므로, 여러 번역 파일로 나뉜 프로젝트도 키를 한데 모으지 않고 파일 단위로 검사합니다.
...
경로 모양만으로는 판단할 수 없습니다. `a/ko.json`과 `ko/common.json`은 둘 다 두 조각이기 때문입니다. 그래서 **어느 조각이 실제 로케일 코드인지**를 보고 결정합니다.
...
그래서 `common.json`에만 있는 키가 `errors.json`에서 누락된 것으로 보고되지 않습니다. 그룹이 하나뿐인 프로젝트(폴더 루트의 일반적인 `single` 구조)에서는 그룹 이름이 `''`이며, CLI는 빈 `@`를 출력하는 대신 아예 생략합니다.
...
`exclude`는 기본 목록에 **추가**하는 것이 아니라 **대체**합니다. 계속 제외하고 싶은 기본값이 있다면 함께 적어야 합니다. 구분자가 들어간 항목은 어느 깊이의 이름이 아니라 검사 루트에서 시작하는 경로(`src/legacy`)를 가리키며, 파일 이름은 `excludeFiles`가 같은 일을 합니다.

읽었지만 로케일에 속하지 않는 파일은 아무 표시 없이 건너뜁니다. `--debug`를 주면 그 파일을 모두 출력하므로, 검사 결과가 비어 있는 이유를 가장 빨리 확인할 수 있습니다.
