...
chki18n은 세 언어로 배포되며, 셋이 아니라 하나의 라이브러리입니다. 같은 25가지 검사를 같은 순서로 수행하고, 옵션 이름도 같고, 리포트도 열 위치까지 같으며, 종료 코드도 같습니다. 프로젝트가 이미 쓰는 언어를 고르면 됩니다. 사이드바 위쪽의 전환 버튼을 누르면 이 사이트의 모든 예제 코드가 그 언어로 바뀝니다.
...
모든 함수와 옵션은 **JavaScript 이름**으로 설명합니다. 제목과 앵커는 어느 독자에게나 같아야 하기 때문입니다. 나머지 둘은 각각 기계적인 규칙 하나만 따르며, 그 규칙을 적는 곳은 여기뿐입니다.
