...
`chki18n` 명령은 번역 파일 폴더를 검사하고 잘못된 부분을 출력합니다. 오류 수준의 이슈를 찾으면 종료 코드 `1`로 끝나므로, CI 작업이나 pre-commit 훅에 그대로 연결할 수 있습니다.
...
디렉터리는 인자로 바로 넘기거나 `--path`로 지정할 수 있으며, 둘은 같은 의미입니다. 상대 경로는 현재 작업 디렉터리를 기준으로 해석됩니다.
...
번역가가 한 번에 다루는 단위가 언어이므로 구획은 언어별로 나뉩니다. 구획 안에서는 검사별로 묶이고 심각한 것이 위로 옵니다. 검사 코드 뒤의 숫자는 발생 횟수이고, 그 아래 한 줄은 검사의 의미입니다. 각 줄에는 키와 함께 비교 대상인 기준 언어의 값이 나옵니다. 검사가 더 구체적인 설명을 만들었다면 다음 줄에 붙습니다. 비교 대상 파일 묶음이 둘 이상이면 키 뒤에 `@`로 그룹이 따라옵니다.

요약은 구획이 보여 주지 않는 축을 채웁니다. 구획을 언어별로 나눴다면 집계는 검사별로, 검사별로 나눴다면 집계는 언어별로 나옵니다.

## 출력 형식 고르기

`--reporter`가 리포트의 모양을 정합니다. 어떤 형식이든 같은 이슈를 같은 순서로 담으며, 그 주변의 글만 달라집니다.
...
`pretty`가 아닌 형식은 배너도 진행 상황 줄도 없이 리포트만 출력합니다. 다른 프로그램으로 그대로 넘길 수 있습니다.

```bash
chki18n ./locales --target en --reporter json > report.json
```

`--debug`는 표준 출력이 아니라 표준 오류로 나가므로, 넘긴 리포트에 섞이지 않습니다.

## 터미널 너비에 맞추기

리포트는 터미널 너비에 맞춰 배치됩니다. 측정할 터미널이 없으면 `COLUMNS` 값을 쓰며, CI 러너는 보통 이 값으로 로그 너비를 알려줍니다. 측정한 너비는 120칸까지만 씁니다. 그보다 벌어지면 라벨과 집계가 한 줄로 읽히지 않기 때문입니다.

`--width`를 주면 이 계산을 모두 무시하며, 상한도 없습니다.
...
구획은 심각한 것부터 놓입니다. 오류가 있는 구획, 경고만 있는 구획, 나머지 순입니다. 구획 안에서도 같은 순서를 따르고, 그다음은 검사 순서, 그다음은 키 순서입니다. 파일이 바뀌지 않았다면 두 번 실행해도 같은 줄이 같은 자리에 오므로, 저장한 리포트끼리 비교할 수 있습니다.

`list`, `json`, `markdown`도 이 기준을 따릅니다. `list`는 구획이 없으니 순서에만 반영됩니다.

## 리포트를 파일로 저장하기

`--output`은 터미널에 출력하면서 같은 리포트를 파일로도 씁니다. 형식은 확장자가 정합니다. `.json`과 `.md`는 각자의 형식이 있고, 나머지는 평문으로 씁니다.

```bash
chki18n ./locales --target en --output translation-report.md
```

없는 디렉터리는 만들어 줍니다. 파일에는 색상 코드가 들어가지 않으며, 터미널 너비가 아니라 고정 너비로 배치되므로 어디서 실행하든 같은 파일이 나옵니다.

둘 다 주면 `--reporter`가 우선합니다. 화면과 파일의 형식을 다르게 하거나, 확장자가 뜻하지 않는 형식을 강제할 때 쓰면 됩니다.

```bash
chki18n ./locales --target en --output report.txt --reporter list
```

리포트를 쓰지 못하면 다른 오류와 마찬가지로 실행이 실패합니다. 만들어지지 않은 파일을 만들어졌다고 보고하지 않습니다.
...
경고만으로는 실행이 실패하지 않습니다. 프로젝트에서 특정 경고를 차단 요소로 취급한다면 `--levels`로 심각도를 올리세요.
...
`error`는 error 주석, `warn`은 warning, `info`는 notice가 됩니다. 줄 번호는 붙이지 않고 파일만 가리킵니다. 검사가 파싱된 번역을 다루는 데다, 가장 흔한 이슈가 애초에 파일에 없는 키이기 때문입니다.

작업 요약에는 마크다운 리포트가 어울립니다.
...
`--no-info`는 머리말 블록과 요약을 없애고 이슈만 남깁니다. 문제가 없을 때는 조용해야 하는 훅에 알맞습니다. `--no-warn`은 여기서 한 걸음 더 나아가 실행을 실패로 만드는 이슈만 남기며, 몇 건을 감췄는지는 리포트에 표시됩니다.
