...
번역 파일은 조용히 깨집니다. 아무것도 죽지 않고 테스트도 통과하며, 빠진 키는 그대로 배포됩니다. 누군가 그 언어로 앱을 열어보기 전까지는 아무도 모릅니다. 모든 풀 리퀘스트에서 chki18n을 돌리면 그 순간이 빌드 실패로 바뀝니다.

설정할 것은 없습니다. 경로와 기준 언어를 주면 나머지는 종료 코드가 합니다. 이 페이지에는 GitHub Actions와 Bitbucket Pipelines에 그대로 붙여 넣을 수 있는 작업이 세 언어별로 정리되어 있습니다.
...
경고는 빌드를 실패시키지 않습니다. 의도한 동작입니다. 경고는 고칠 값어치는 있어도 릴리스를 막을 일은 아니며, 덕분에 이미 굴러가는 프로젝트에 하루치 정리 작업 없이 바로 붙일 수 있습니다. 프로젝트에서 차단 요소로 보는 항목은 [`--levels`](./options#levels)로 승격하세요.
...
`if: always()`가 핵심입니다. 이 줄이 없으면, 정작 리포트를 읽어야 할 때 업로드가 건너뛰어집니다.
...
돌아가고 있는 프로젝트에 처음 켜면, 그날 오후에 다 고칠 수 있는 양보다 많이 나오는 것이 보통입니다. 그렇다고 빌드를 막을 필요는 없습니다.

이미 동의하는 검사부터 시작해서, 정리되는 대로 목록을 늘려가면 됩니다.

```bash
chki18n ./locales --target en --checks NO_KEY,NO_INTERPOLATION_KEY
```

반대로 전부에서 시작해 시끄러운 것만 빼도 됩니다.

```bash
chki18n ./locales --target en --ignore-checks DUPLICATE_VALUE
```

검사는 모두 두고 무엇을 오류로 볼지만 정하는 방법도 있습니다.
...
프로젝트가 원하는 기준이 "무엇이든 실패했는가"가 아니라면 — 개수 기준선, 언어별 규칙, 어딘가에 코멘트 남기기 — 종료 코드 대신 결과를 읽으세요.
...
라이브러리는 프로세스를 종료하지 않고, 요청하지 않으면 출력도 하지 않습니다. 둘 다 직접 만든 스크립트가 정합니다. [`checkTranslationFiles`](/ko/api/check-translation-files)와 [결과 객체](/ko/reference/result)를 참고하세요.
