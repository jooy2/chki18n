...
hero:
  name: chki18n
  text: 번역 파일, 제대로 검사하기
  tagline: 누락된 키, 비어 있는 값, 번역되지 않은 문자열, 더 이상 맞지 않는 보간 키. 모든 i18n JSON 구조에 대해 11가지를 검사하며, 커맨드라인에서도 코드에서도 동작합니다.
  actions:
    - theme: brand
      text: 시작하기
...
features:
  - title: 11가지 검사, 하나의 리포트
    details: 누락된 키, 비어 있는 값, 번역되지 않은 문자열, 어긋난 보간 키, 중복된 값, 앞뒤 공백, 번역에서 빠진 숫자까지. 각 검사의 심각도는 프로젝트에 맞게 조정할 수 있습니다.
    link: /ko/guide/checks
    linkText: 검사 항목
